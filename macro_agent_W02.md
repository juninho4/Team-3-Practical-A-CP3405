Week of 25 May 2026

FED & RATES (CME FedWatch + Treasury.gov):
 · Current Fed rate: [3.50–3.75%]
 · Next FOMC date: [17 Jun 2026]. Hold probability: [99.6%]. Cut probability: [0.4%]. Direction vs last week: [unchanged]
 · 2-year yield: [3.98%]   10-year yield: [4.45%]   30-year yield: [4.99%]
 · Yield curve: [normal]. 10-year direction this week: [rising / falling / flat]
 · [Implication: rising 10-year = headwind for growth stocks / falling 10-year = relief for REITs and tech]

COMMODITIES & DOLLAR (Finviz Futures 1W):
 · WTI Crude Oil: [close price], weekly change: [-9.57%%], direction: [falling]
 · Gold: [close price], weekly change: [+0.81%], direction: [rising]
 · DXY (Dollar): [-0.34%], direction: [weakening]
 · [Cross-asset implication: oil rising + yields rising = stagflation risk. Oil falling = inflation relief.]

WEEK-AHEAD CALENDAR (TradingEconomics):
 · [Sunday, 31 May]: [NBS  Manufacturing  PMI] — Expected: [50.5], Previous: [50.3] — IMPORTANCE: [High]
 · [Tuesday, 02 June]: [Inflation Rate YoY Flash ] — Expected: [3.4%], Previous: [3%] — IMPORTANCE: [High]
 · [Repeat for each major event this week. Maximum 5 entries. Rank by market-moving potential.]

KEY EARNINGS THIS WEEK (Earnings Whispers):
 · [CRWD (TICKER)] — 31 May — Sector: [IT] — What to watch: [+8.94%]
 . [HPE (TICKER)] — 31 May — Sector: [IT] — What to watch: [+10.3% expectation beat]
 · [2–3 entries maximum. Only include earnings that could move an entire sector.]

CONFIRMED NEWS EVENTS (Reuters / AP):
 · [China factory activity stalls in May as demand weakens] — Source: Reuters — 31 May — Market implication: [China as a global supplier may affect outputs on the rest of the globalb economy]
 . [Pentagon chief says U.S. ready to restart strikes on Iran if no deal] -Source: Reuters — 30 May - Market implication: [Oil shipping lanes will remain closed and industry will be redirected to war efforts]

 · [Confirmed facts only. No rumours. No social media. If you cannot cite Reuters or AP, leave it out.]

MACRO BIAS: [Hawkish]
PRIMARY DRIVER THIS WEEK: [Potential restarting of war in the middle east]
CONFIDENCE: [Medium] — [Peace talks could go either way, terms may be favourable for both parties]
INVALIDATION: [Peace talks are settled upon for the war in Iran]
