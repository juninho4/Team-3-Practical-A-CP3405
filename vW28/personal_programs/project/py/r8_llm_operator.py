...
- output/agents/almanac/                    R3 Markdown evidence
- output/agents/                             R4 Markdown evidence
- output/technical agent/                    R5 CSV evidence
- output/llm/                                prompt, AI answers and comparison

One run (API mode is the default):
1. Read R3/R4 Markdown and R5 CSV from output/
2. Generate output/llm/shared_prompt.md
3. Call the Gemini API (default)
4. Save the Gemini answer into output/llm/
5. Generate output/llm/llm_comparison.md from the current answer

Other providers can still be selected explicitly from the command line.
Browser automation is retained as an optional fallback with --mode browser.
"""
...
def input_file_candidates(root: Path, sprint: str) -> dict[str, list[Path]]:
    agents_output = root / "output" / "agents"
    almanac_output = agents_output / "almanac"
    technical_output = root / "output" / "technical agent"
    return {
        "r3": [
            almanac_output / f"{sprint.lower()}_almanac.md",
        ],
        "r4": [
            agents_output / f"macro_agent_output_{sprint}.md",
        ],
...
def find_input_file(root: Path, sprint: str, key: str) -> Optional[Path]:
    ...
    if key == "r5":
        files = find_r5_csv_files(root / "output" / "technical agent")
        return files[0] if files else None

    search_locations = {
        "r3": (
            root / "output" / "agents" / "almanac",
            ["*_almanac.md"],
        ),
        "r4": (
            root / "output" / "agents",
        ...
        "r5": (root / "output" / "technical agent", []),
    }
...
def load_inputs(root: Path, sprint: str) -> tuple[str, str, str, list[str]]:
    ...
    for key, label in labels.items():
        if key == "r5":
            paths = find_r5_csv_files(root / "output" / "technical agent")
            if not paths:
                expected = "CSV files under output/technical agent/"
        ...
        if path is None:
            expected_locations = {
                "r3": "output/agents/almanac/*_almanac.md",
                "r4": "output/agents/macro_agent_output_W*.md",
                "r5": "a CSV under output/technical agent/ containing Ticker and Technical Bias columns",
            }
...
def generate_question(root: Path, sprint: str) -> Path:
    ...
    path = root / "output" / "llm" / "shared_prompt.md"
...
def save_answer(root: Path, site: AISite, sprint: str, page: Page, prompt_text_for_site: str) -> None:
    output_path = root / "output" / "llm" / site.output_filename
...
def extract_deepseek_text(response: dict[str, Any]) -> str:
...
def call_api_provider(
    ...
    if provider.key == "chatgpt":
        ...
        answer = extract_anthropic_text(response)
        ...
        answer = extract_deepseek_text(response)
    else:
...
def load_model_results(root: Path, sprint: str) -> list[ModelResult]:
    ...
    for model in MODELS:
        path = root / "output" / "llm" / f"synthesis_{model}.txt"
        if not path.exists():
            missing.append(path)
            continue
...
def generate_comparison_md(sprint: str, results: list[ModelResult]) -> str:
    ...
    raw_paths = "\n".join(
        f"- `output/llm/synthesis_{model}.txt`" for model in available_models
    )
...
def summarize(root: Path, sprint: str) -> None:
    ...
    paths = [
        root / "output" / "llm" / "shared_prompt.md",
        root / "output" / "llm" / "api_call_log.md",
        root / "output" / "llm" / f"synthesis_chatgpt.txt",
        root / "output" / "llm" / f"synthesis_deepseek.txt",
        root / "output" / "llm" / f"synthesis_gemini.txt",
        root / "output" / "llm" / f"synthesis_claude.txt",
    ...
    print(f"- R8 output folder:     {root / 'output' / 'llm'}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="gemini",
        help="AI provider to call (default: Gemini only)",
    )
    parser.add_argument(
        "--mode",
        choices=["api", "browser"],
        default="api",
        help="Use provider APIs (default) or the legacy Edge browser workflow",
    )
