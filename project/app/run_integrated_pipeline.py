...
DASHBOARD_EXPORT_SCRIPT = PY_DIR / "r9_dashboard_export.py"
...
def verify_api_configuration(only: str) -> None:
    """Report missing API keys without stopping the integrated pipeline."""
    ...
    print(f"R8 API preflight: {len(configured)}/{len(enabled)} configured provider(s) found.")
    if missing_names:
        print(
            "R8 API preflight warning [CONFIG_MISSING]: "
            + ", ".join(missing_names)
            + f". R8 will log the skipped providers and continue. Optional local file: {env_path}"
        )
...
def main() -> int:
    ...
    try:
        ...
        os.environ["MARKET_WEEK_OUTPUT_DIR"] = str(week_output.resolve())
        ...
        heading(7, total, "Dashboard — export latest JSON for GitHub Pages")
        run_python(
            DASHBOARD_EXPORT_SCRIPT,
            [
                "--week",
                ...
                str(week_output.resolve()),
                "--docs-dir",
                str((REPOSITORY_ROOT / "docs").resolve()),
            ],
        )
