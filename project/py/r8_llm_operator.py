...
_WEEK_OUTPUT_OVERRIDE: Optional[Path] = None


def configure_week_output_root(
    root: Path,
    sprint: str,
    requested_path: Optional[str],
) -> Path:
    """Set one validated week folder used by every R8 read/write operation.

    The integrated launcher passes the exact week folder, such as repository/vW31.
    This prevents one provider (for example Gemini) from accidentally writing to a
    stale or project-relative path while the rest of the pipeline uses another week.
    """
    global _WEEK_OUTPUT_OVERRIDE

    label = normalize_week_label(sprint)
    raw = (requested_path or os.environ.get("MARKET_WEEK_OUTPUT_DIR", "")).strip()
    if raw:
        ...
        if candidate.name.casefold() != label.casefold():
            raise RuntimeError(
                "Week output directory does not match the requested sprint: "
                f"sprint={label}, directory={candidate}"
            )
    else:
        candidate = (root.parent / label).resolve()

    candidate.mkdir(parents=True, exist_ok=True)
    _WEEK_OUTPUT_OVERRIDE = candidate
    os.environ["MARKET_WEEK_OUTPUT_DIR"] = str(candidate)
    return candidate


def weekly_output_root(root: Path, sprint: str) -> Path:
    """Return the single validated repository week folder for this R8 run."""
    label = normalize_week_label(sprint)
    if _WEEK_OUTPUT_OVERRIDE is not None:
        if _WEEK_OUTPUT_OVERRIDE.name.casefold() != label.casefold():
            raise RuntimeError(
                "Configured week output directory no longer matches sprint: "
                f"sprint={label}, directory={_WEEK_OUTPUT_OVERRIDE}"
            )
        return _WEEK_OUTPUT_OVERRIDE

    raw = os.environ.get("MARKET_WEEK_OUTPUT_DIR", "").strip()
    if raw:
        return configure_week_output_root(root, label, raw)
    return (root.parent / label).resolve()
...
def display_path(path: Path, root: Path) -> str:
    """Return a repository-relative path without raising for sibling week folders."""
...
def api_error_code(exc: Exception) -> tuple[str, str]:
    """Return a compact machine-readable error code and a safe detail string."""
    detail = str(exc).strip() or exc.__class__.__name__
    http_match = re.search(r"\bHTTP\s+(\d{3})\b", detail, flags=re.IGNORECASE)
    provider_match = re.search(
        r"\berror\s+code\s*[:=]\s*([A-Za-z0-9_.-]+)",
        detail,
        flags=re.IGNORECASE,
    )
    if http_match and provider_match:
        return f"HTTP_{http_match.group(1)} / {provider_match.group(1)}", detail
    if http_match:
        return f"HTTP_{http_match.group(1)}", detail
    lowered = detail.casefold()
    if "network error" in lowered or "timed out" in lowered or "timeout" in lowered:
        return "NETWORK_ERROR", detail
    if "did not contain answer text" in lowered or "returned no answer text" in lowered:
        return "EMPTY_RESPONSE", detail
    if "not in the subpath" in lowered or "relative_to" in lowered:
        return "PATH_ERROR", detail
    if "non-json" in lowered:
        return "NON_JSON_RESPONSE", detail
    if "unexpected json" in lowered:
        return "UNEXPECTED_RESPONSE", detail
    return exc.__class__.__name__.upper(), detail
    ...
    try:
        ...
        try:
            ...
            if isinstance(error_value, dict):
                ...
                provider_code = error_value.get("code", "")
            else:
                message = error_value
            ...
            message = body[:500]
        code_suffix = f" (error code: {provider_code})" if provider_code else ""
        raise RuntimeError(
            f"HTTP {exc.code}: {message or exc.reason}{code_suffix}"
        ) from exc
...
def extract_gemini_text(response: dict[str, Any]) -> str:
    """Extract text from both Gemini Interactions and generateContent responses."""
    direct = response.get("output_text")
    if isinstance(direct, str) and direct.strip():
        return direct.strip()

    # Interactions API REST responses return model text in
    # steps[].content[].text (the SDK-only output_text helper may be absent).
    pieces: list[str] = []
    ...
    # generateContent fallback responses return candidates[].content.parts[].text.
...
def gemini_response_diagnostic(response: dict[str, Any]) -> str:
    """Return a compact, secret-free explanation for an empty Gemini response."""
    details: list[str] = []
    ...
    if step_types:
        details.append("steps=" + ",".join(step_types))
    ...
    if isinstance(error, dict) and error.get("message"):
        details.append(f"error={str(error['message'])[:160]}")

    return "; ".join(details) if details else "no text-bearing fields returned"
...
def call_api_provider(
    ...
    if provider.key == "groq":
        ...
        # First use the current Interactions API. REST responses expose text in
        # steps[].content[].text rather than the generateContent candidates shape.
        response = post_json(
        ...
        # Some Gemini calls can occasionally complete without a text block.
        # Retry through generateContent, whose response shape is also supported.
        if not answer:
            diagnostic = gemini_response_diagnostic(response)
            print(
                "[R8] Gemini Interactions response had no text "
                f"({diagnostic}); retrying with generateContent."
            )
            ...
            answer = extract_gemini_text(fallback)
            if not answer:
                raise RuntimeError(
                    "Gemini returned no answer text after Interactions and "
                    "generateContent attempts; "
                    + gemini_response_diagnostic(fallback)
                )
...
def ask_all_api(
    ...
    """Call every enabled API and always return control to the pipeline.

    Provider failures are printed with an error code and written to
    api_call_log.md. They never raise out of this function.
    """
    print("")
    print("=" * 70)
    ...
    requested_minimum = max(0, minimum_success)
    if requested_minimum > len(enabled):
        print(
            "[R8] WARNING: --min-api-success exceeds enabled providers; "
            "the value is informational because API failures are non-fatal."
        )
    ...
    for provider in enabled:
        ...
        if not api_key:
            code = "CONFIG_MISSING"
            detail = f"missing {provider.key_env}"
            status = "SKIPPED"
            print(f"[R8] {provider.name} API skipped [{code}]: {detail}")
            log_rows.append((provider.name, model, status, code, detail, "-"))
            continue
        ...
        try:
            ...
            successes += 1
            relative = display_path(output_path, root)
            print(f"[R8] Saved API answer: {output_path}")
            log_rows.append((provider.name, model, "OK", "-", "-", relative))
        except Exception as exc:
            # Keep running so the remaining providers and report still execute.
            code, detail = api_error_code(exc)
            safe_detail = detail[:500]
            print(f"[R8] {provider.name} API failed [{code}]: {safe_detail}")
            log_rows.append(
                (provider.name, model, "FAILED", code, safe_detail, "-")
            )
    ...
    lines = [
        ...
        f"- Requested minimum successes: {requested_minimum}",
        "- Failure policy: non-fatal; errors are logged and the pipeline continues",
        "",
        "| Provider | Model | Status | Error Code | Detail | Output |",
        "|---|---|---|---|---|---|",
    ]
    for name, model, status, code, detail, output in log_rows:
        cells = [name, model, status, code, detail, output]
        safe_cells = [
    ...
    if successes < requested_minimum:
        print(
            f"[R8] WARNING: Only {successes} API provider(s) succeeded; "
            f"requested minimum was {requested_minimum}. Continuing without exit."
        )
...
def generate_report(root: Path, sprint: str) -> Path:
    ...
    if not results:
        log_path = weekly_output_root(root, sprint) / "llm" / "api_call_log.md"
        log_text = read_text(log_path)
        report = (
            f"# R8 LLM Comparison - {sprint}\n\n"
            "## Status\n\n"
            "No usable AI response files were produced in this run. "
            "The API errors were recorded and the pipeline continued.\n\n"
            "## API Call Log\n\n"
            + (log_text if log_text else "API call log was not available.")
            + "\n"
        )
        write_text(output, report)
        print(f"[R8] Final report generated with API errors only: {output}")
        return output
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Exact repository week folder, such as /repo/vW31. "
            "All R8 providers and reports use this same validated path."
        ),
    )
    parser.add_argument(
        "--only",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Informational success target; API failures never terminate R8",
    )
    ...
    week_output = configure_week_output_root(
        root=root,
        sprint=args.sprint,
        requested_path=args.week_output_dir,
    )
    ensure_dirs(root, args.sprint)

    print(f"[R8] Project root: {root}")
    print(f"[R8] Sprint: {args.sprint}")
    print(f"[R8] Week output folder: {week_output}")
